...
/**
 * @param {object} props
 * @param {object | null} props.keys - Valid RSA key pair or null.
 * @param {string | null} props.error - Key-generation error message.
 * @param {{ p: string, q: string, e: string }} [props.inputPreview] - Raw field values when keys are invalid.
 */
export function KeyGenVisualization({ keys, error, inputPreview }) {
  const [showDSteps, setShowDSteps] = useState(false)

  const phiMultiples = useMemo(() => {
    if (!keys) return null
    return getDByPhiMultiplesSteps(keys.phi, keys.e)
  }, [keys])
  ...
  useEffect(() => {
    if (!keys) setShowDSteps(false)
  }, [keys])

  const pShow = inputPreview?.p?.trim() ? inputPreview.p.trim() : '—'
  const qShow = inputPreview?.q?.trim() ? inputPreview.q.trim() : '—'
  const eShow = inputPreview?.e?.trim() ? inputPreview.e.trim() : '—'

  const showFlow = Boolean(keys || error)
  ...
  return (
    <div className={styles.wrapper}>
      ...
      {!showFlow && <p className={styles.placeholder}>Enter p, q and e to generate keys.</p>}
          ...
          <div className={styles.flow}>
            <div className={styles.step}>
              ...
              <div className={styles.formula}>
                <code>p = {keys ? keys.p.toString() : pShow}</code>
                <code>q = {keys ? keys.q.toString() : qShow}</code>
              </div>
            </div>
            ...
            <div className={styles.step}>
              ...
              <div className={styles.formula}>
                <code>n = p × q = {keys ? keys.n.toString() : '—'}</code>
              </div>
            </div>
            <div className={styles.arrow}>↓</div>
            <div className={styles.step}>
              ...
              <div className={styles.formula}>
                <code>φ(n) = (p−1)(q−1) = {keys ? keys.phi.toString() : '—'}</code>
              </div>
            </div>
            <div className={styles.arrow}>↓</div>
            <div className={styles.step}>
              ...
              <div className={styles.formula}>
                <code>e = {keys ? keys.e.toString() : eShow}</code>
                <code>d ≡ e⁻¹ (mod φ(n)) = {keys ? keys.d.toString() : '—'}</code>
              </div>
            </div>
          </div>

          {showDSteps && keys && phiMultiples && (
            <div className={styles.dStepsPanel}>
              ...
              <div className={styles.dStepBlock}>
                ...
                <p>
                  φ(n) = <strong>{keys.phi.toString()}</strong>, e = <strong>{keys.e.toString()}</strong>. For each k, form <strong>k · φ(n)</strong>, add 1, and check whether the result is divisible by e.
                </p>
              </div>
                ...
                <>
                  {attemptChunks.mode === 'all'
                    ? attemptChunks.rows.map((row) => (
                        <PhiAttemptCard key={row.k.toString()} row={row} e={keys.e} />
                      ))
                    : (
                        <>
          ...
          {keys && (
            <div className={styles.keys}>
              <div className={styles.keyBox}>
                ...
                <code>(e, n) = ({keys.e.toString()}, {keys.n.toString()})</code>
              </div>
              <div className={styles.keyBox}>
                <span className={styles.keyTitle}>🔐 Private key</span>
                <code>(d, n) = ({keys.d.toString()}, {keys.n.toString()})</code>
              </div>
            </div>
          )}
        </>
